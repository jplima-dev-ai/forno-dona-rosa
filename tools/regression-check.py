...
check("All catalog products have images", sum(1 for item in catalog_data.get('products',[]) if isinstance(item,dict) and str(item.get('image','')).startswith('assets/images/products/')) == len(catalog_data.get('products',[])))
...
check("Optional order fields progressive", ('id="configurator-step-2"' in html and 'id="remove-ingredients"' in html and 'id="notes"' in html) or '<details class="order-extras">' in html)
