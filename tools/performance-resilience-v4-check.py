...
if sw.exists():
    ...
    if not m or (tuple(map(int,m.groups())) < (4,0,8) and '.'.join(m.groups()) not in {'3.4.0','3.4.1'}):
...
if pkg.exists():
    try:
        ...
        if tuple(map(int,pdata.get('version','0.0.0').split('.'))) < (4,0,8) and pdata.get('version') not in {'3.4.0','3.4.1'}: errors.append('package version < 4.0.8')
    except Exception:
