...
check('Bag schema v4', 'bag-v4' in main and 'bag-v3' in main and 'schemaVersion: BAG_SCHEMA_VERSION' in main and 'storageNamespace' in main)
...
ids=[item.get('id') for item in catalog.get('products',[]) if isinstance(item,dict)]
...
check('Progressive order extras', ('id="configurator-step-2"' in html and 'id="remove-ingredients"' in html and 'id="notes"' in html) or '<details class="order-extras">' in html)
...
check('Product image baseline', len(product_images)>=len(json.loads((ROOT/'data/catalog.json').read_text(encoding='utf-8'))['products']), str(len(product_images)))
check('Every menu item has product image', sum(1 for item in catalog.get('products',[]) if isinstance(item,dict) and str(item.get('image','')).startswith('assets/images/products/'))==len(ids), f"{sum(1 for item in catalog.get('products',[]) if isinstance(item,dict) and str(item.get('image','')).startswith('assets/images/products/'))}/{len(ids)}")
...
check('Responsive product variants', len(list((ROOT/'assets/images/products').glob('*-384.webp')))>=len(json.loads((ROOT/'data/catalog.json').read_text(encoding='utf-8'))['products']), str(len(list((ROOT/'assets/images/products').glob('*-384.webp')))))
...
check('Storage namespace', 'storageNamespace' in (ROOT/'js/app-config.js').read_text(encoding='utf-8') and 'storageNamespace' in main and 'assistant-session-v5' in rosa)
