...
CREDIT_NAME = CREDITS.get('name', 'Kael J Productions')
...
def image_dimensions(rel_path: str) -> tuple[int, int]:
    path = ROOT / rel_path
    try:
...
def image_size_attrs(rel_path: str) -> str:
    width, height = image_dimensions(rel_path)
...
def experience_page():
    ...
    body=f'''<section class="page-hero page-hero--experience"><div class="section-shell">{breadcrumb(1,[('Início',''),('Experiência',None)])}<p class="kicker">Da farinha ao fogo</p><h1>{escape(CONTENT.get('site',{}).get('experience',{}).get('title','O que acontece antes da primeira mordida.'))}</h1><p>{escape(CONTENT.get('site',{}).get('experience',{}).get('lead','Uma sequência simples de escolhas que cria a assinatura sensorial da casa.'))}</p></div></section><section class="section"><div class="section-shell"><ol class="experience-timeline">{items}</ol></div></section><section class="section page-cta"><div class="section-shell"><h2>Agora escolha o seu sabor.</h2><a class="btn btn--primary" href="{url_for(1,'menu/')}">Explorar o cardápio</a></div></section>'''
    return page('Experiência',TECHNICAL_PAGES['experience'][1],body,depth=1,current='experience',canonical_path='experience/')


def location_page():
    body=f'''<section class="page-hero page-hero--compact"><div class="section-shell">{breadcrumb(1,[('Início',''),('Localização',None)])}<p class="kicker">Visite a Dona Rosa</p><h1>{escape(CONTENT.get('site',{}).get('location',{}).get('title','Forno aceso em Laranjeiras, Serra.'))}</h1><p>{escape(CONTENT.get('site',{}).get('location',{}).get('lead','Retire seu pedido na pizzaria ou confirme a entrega pelo WhatsApp.'))}</p></div></section><section class="section"><div class="section-shell location-page__grid"><article class="local-card"><h2>Endereço</h2><p>{escape(ADDRESS)}</p><a class="btn btn--primary" id="route-link" href="#">Abrir rota</a></article><article class="local-card"><h2>Horários</h2><p class="business-status-inline"><strong data-business-status>Verificando horário…</strong><span data-business-status-detail></span></p><p data-brand-hours>Segunda a sexta, das 18h à 0h. Sábados e domingos, das 16h à 0h.</p><p id="hours-note"></p></article><article class="local-card"><h2>Atendimento</h2><p>WhatsApp: {escape(WHATSAPP)}</p><p><a href="mailto:{escape(EMAIL)}">{escape(EMAIL)}</a></p></article><article class="local-card"><h2>Retirada</h2><p>Escolha “Retirada na pizzaria” ao finalizar o pedido e busque no endereço da Dona Rosa.</p><a class="btn btn--ghost" href="{url_for(1,'order/')}">Fazer pedido para retirada</a></article></div></section>'''
...
def help_page():
    faqs=[('Onde vocês entregam?','Em Serra — ES. O CEP é validado no checkout para pedidos de entrega.'),('Posso retirar?','Sim. Ao escolher retirada, o checkout remove os campos de CEP e endereço e mostra o endereço da pizzaria.'),('Quais pagamentos são aceitos?','Pix e dinheiro em espécie. Se precisar de troco, informe o valor no checkout.'),('Posso agendar?','Sim. Escolha um horário disponível dentro do funcionamento configurado.'),('Posso pedir molhos?','Sim. No final do checkout você pode selecionar molhos disponíveis, como maionese, ketchup, mostarda, molho de alho e molho picante.'),('Como confirmo o pedido?','Ao final, abrimos o WhatsApp com o resumo pronto para você conferir e confirmar com a pizzaria.')]
    blocks=''.join(f'<details class="faq-block"><summary>{escape(q)}</summary><p>{escape(a)}</p></details>' for q,a in faqs)
    body=f'''<section class="page-hero page-hero--compact"><div class="section-shell">{breadcrumb(1,[('Início',''),('Ajuda',None)])}<p class="kicker">Ajuda</p><h1>{escape(CONTENT.get('site',{}).get('help',{}).get('title','Respostas rápidas antes do pedido.'))}</h1><p>{escape(CONTENT.get('site',{}).get('help',{}).get('lead','Tudo o que você precisa saber antes de pedir.'))}</p></div></section><section class="section"><div class="section-shell help-page">{blocks}<div class="help-page__cta"><p>Ainda ficou alguma dúvida?</p><button class="btn btn--rosa" data-rosa-context="ajuda" data-rosa-open type="button">Perguntar à Rosa</button></div></div></section>'''
    return page('Ajuda',TECHNICAL_PAGES['help'][1],body,depth=1,current='help',canonical_path='help/')


def privacy_page():
    body=f'''<section class="page-hero page-hero--compact"><div class="section-shell narrow-copy">{breadcrumb(1,[('Início',''),('Privacidade',None)])}<p class="kicker">Privacidade</p><h1>Seu pedido continua sob seu controle.</h1><p>Aqui explicamos de forma clara como suas informações são usadas durante o pedido.</p></div></section><section class="section"><div class="section-shell legal-copy"><h2>Dados do checkout</h2><p>Nome e endereço ficam na sessão do navegador por padrão. Eles só são lembrados no dispositivo se você marcar explicitamente essa opção.</p><h2>Consulta de CEP</h2><p>Somente o CEP é enviado aos provedores configurados de consulta postal. Nome, número, complemento e referência não são enviados a esses serviços.</p><h2>WhatsApp</h2><p>Ao finalizar, o site abre o WhatsApp com o resumo preparado para você revisar e confirmar com a pizzaria.</p><h2>Medição de uso</h2><p>Não utilizamos rastreamento de comportamento nesta experiência enquanto esse recurso permanecer desativado.</p></div></section>'''
...
def product_page(product:dict):
    slug=product['id']; depth=2; media_contract=product.get('media',{}) or {}; hero_contract=media_contract.get('hero',{}) or {}; catalog_contract=media_contract.get('catalog',{}) or {}; img=hero_contract.get('src') or product['image']; available=slug not in UNAVAILABLE
    media_alt=hero_contract.get('alt') or catalog_contract.get('alt') or f"{product['name']} — {product.get('description','')}"
    temperature=media_contract.get('temperature','cold' if product.get('type')=='bebida' else 'hot')
    ...
    reason_titles={
        'dona-rosa':'A assinatura da casa.',
        'calabresa':'Um clássico com fogo e personalidade.',
        'quatro-formaggi':'Cremosidade em quatro camadas de sabor.',
        'margherita':'Poucos ingredientes, muito equilíbrio.',
        'nordestina-dona-rosa':'Sabor brasileiro com assinatura da casa.',
        'agua':'Leve para acompanhar sem competir com o sabor.',
        'agua-gas':'Frescor e leveza entre uma fatia e outra.',
        'suco-laranja':'Fresco, cítrico e fácil de combinar.'
    }
    if slug in reason_titles:
        reason_title=reason_titles[slug]
    elif product['type']=='bebida':
        reason_title='A companhia certa para o seu pedido.'
    elif product.get('category')=='doces':
        reason_title='Um final que merece espaço.'
    elif 'vegana' in trait_set or 'vegetariana' in trait_set:
        reason_title='Sabor completo, sem carne.'
    elif 'picante' in trait_set or 'intensa' in trait_set:
        reason_title='Para quem gosta de sabores marcantes.'
    elif 'queijo' in trait_set or 'cremosa' in trait_set:
        reason_title='Cremosa, intensa e feita para dividir.'
    else:
        reason_title='Uma combinação feita para funcionar.'
    ...
    media=f'''<picture class="product-media-v4" data-temperature="{escape(temperature)}"><source srcset="{prefix(depth)}{escape(stem)}-480.avif 480w, {prefix(depth)}{escape(stem)}-800.avif 800w, {prefix(depth)}{escape(stem)}-1200.avif 1200w" sizes="(max-width:48rem) 100vw, 50vw" type="image/avif"><source srcset="{prefix(depth)}{escape(stem)}-480.webp 480w, {prefix(depth)}{escape(stem)}-800.webp 800w, {prefix(depth)}{escape(stem)}-1200.webp 1200w" sizes="(max-width:48rem) 100vw, 50vw" type="image/webp"><img src="{prefix(depth)}{escape(img)}" alt="{escape(media_alt)}" {image_size_attrs(img)} fetchpriority="high" decoding="async" class="focal-media focal-x-{fx:.0f} focal-y-{fy:.0f}"></picture>'''
    body=f'''<section class="product-page"><div class="section-shell">{breadcrumb(depth,[('Início',''),('Cardápio','menu/'),(product['name'],None)])}<div class="product-page__grid"><div class="product-page__media">{media}<span class="product-page__media-note">Foto ilustrativa do produto</span></div><article class="product-page__info"><p class="kicker">{escape(product.get('categoryLabel','Cardápio'))}</p><h1>{escape(product['name'])}</h1><p class="product-page__badge">{escape(merch_label)}</p><p class="product-page__availability" data-availability="{'available' if available else 'unavailable'}">{availability_copy}</p><div class="product-page__price">A partir de <strong>{money(product['basePrice'])}</strong></div><p class="product-page__description">{escape(product['description'])}</p><div class="product-page__traits" aria-label="Características">{traits}</div><div class="product-page__actions">{action_html}</div><ul class="product-confidence" aria-label="Informações do pedido"><li>Confira o valor antes de confirmar</li><li>Entrega ou retirada na pizzaria</li><li>Pix ou dinheiro em espécie</li></ul></article></div><section class="product-reasons" aria-labelledby="reasons-title"><div><p class="kicker">Por que você vai gostar</p><h2 id="reasons-title">{escape(reason_title)}</h2></div><ul>{reasons_html}</ul></section>{pairing_html}<section class="product-alternatives" id="product-alternatives" aria-labelledby="alternatives-title"><p class="kicker">Continue explorando</p><h2 id="alternatives-title">Talvez você também goste.</h2><div>{alternatives_html}</div></section></div></section><section class="section product-page__next"><div class="section-shell"><p class="kicker">Continue escolhendo</p><h2>Complete seu pedido.</h2><div><a class="btn btn--ghost" href="{url_for(depth,'menu/')}">Voltar ao cardápio</a><a class="btn btn--primary" href="{url_for(depth,'order/')}">Ir para o pedido</a></div></div></section>'''
...
def articles_hub_page():
    ...
    if featured:
        ...
        feature_html=f'<article class="article-feature"><div><p class="kicker">{escape(cat.get("label","Em destaque"))}</p><h2><a href="../articles/{escape(featured["slug"])}/">{escape(featured["title"])}</a></h2><p>{escape(featured["summary"])}</p><a class="btn btn--ghost" href="../articles/{escape(featured["slug"])}/">Ler artigo</a></div><img alt="" src="../{escape(featured.get("heroImage","assets/images/dona-rosa-hero-pizza.webp"))}" {image_size_attrs(featured.get("heroImage","assets/images/dona-rosa-hero-pizza.webp"))} decoding="async"></article>'
